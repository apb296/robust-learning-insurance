...
%ComputeValueForSimpleContractAgent1=@(c) (-theta_11*log(pl(1).*exp(-u(c,ra)./theta_11)+ph(1).*exp(-u(c+Delta,ra)./theta_11)));
...
%    ResComputeValueForSimpleContract = @(c) (-theta_21*log(pl(2).*exp(-u(y-c,ra)./theta_21)+ph(2).*exp(-u(y-c-Delta,ra)./theta_21))) -v;
...
[ c ] = (FitConcaveValueFunction(Q,QNew',domain',domain(1:2:end)'))';
%[ c ] = (FitConcaveValueFunction(Q,u_inv(QNew',ra),domain',domain(1:2:end)'))';
