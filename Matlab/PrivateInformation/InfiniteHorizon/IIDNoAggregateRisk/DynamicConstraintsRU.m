...
%Qstar=u(funeval(coeff,Q,bar_vstar',0),ra);
%lambdastar=-funeval(coeff,Q,bar_vstar',1).*der_u(funeval(coeff,Q,bar_vstar'),ra);
...
%gradCEQ=[-der_u(y-c(1),ra)*tilde_p0_agent_2(1)*(1-beta)  -der_u(y-c(2),ra)*tilde_p0_agent_2(2)*(1-beta)  beta*tilde_p0_agent_2(1) beta*tilde_p0_agent_2(2)]';
...
%IC_1_1 = u(c(1),ra)*(1-beta) -u(c(2)-Delta,ra)*(1-beta) +beta*(Qstar(1)-Qstar(2));
IC_1_1 = u(c(1),ra) -u(c(2)-Delta,ra)  +beta*(Qstar(1)-Qstar(2));
% 2) Agent 1, z=z_2
%IC_1_2 = u(c(2),ra)*(1-beta) -u(c(1)+Delta,ra)*(1-beta) +beta*(Qstar(2)-Qstar(1));
IC_1_2 = u(c(2),ra)  -u(c(1)+Delta,ra)  +beta*(Qstar(2)-Qstar(1));

% 3) Agent 2, z=z_1
%IC_2_1 = u(y-c(1),ra)*(1-beta) -u(y-c(2)+Delta,ra)*(1-beta) +beta*(bar_vstar(1)-bar_vstar(2));
IC_2_1 = u(y-c(1),ra)  -u(y-c(2)+Delta,ra)  +beta*(bar_vstar(1)-bar_vstar(2));

% 4) Agent 2, z=z_2
%IC_2_2 = u(y-c(2),ra)*(1-beta) -u(y-c(1)-Delta,ra)*(1-beta) +beta*(bar_vstar(2)-bar_vstar(1));
...
%gradCINQ=-[der_u(c(1),ra)*(1-beta)  -der_u(c(2)-Delta,ra)*(1-beta)  -beta*lambdastar(1)    beta*lambdastar(2) ;
gradCINQ=-[der_u(c(1),ra) -der_u(c(2)-Delta,ra)   -beta*lambdastar(1)    beta*lambdastar(2) ;
    
%-der_u(c(1)+Delta,ra)*(1-beta)  der_u(c(2),ra)*(1-beta)  beta*lambdastar(1)    -beta*lambdastar(2) ;
-der_u(c(1)+Delta,ra) der_u(c(2),ra) beta*lambdastar(1)    -beta*lambdastar(2) ;

%-der_u(y-c(1),ra)*(1-beta)  der_u(y-c(2)+Delta,ra)*(1-beta)  beta    -beta ;
-der_u(y-c(1),ra) der_u(y-c(2)+Delta,ra)   beta    -beta ;

%der_u(y-c(1)-Delta,ra)*(1-beta)  -der_u(y-c(2),ra)*(1-beta)  -beta    beta ;
