...
% Constraints for the ambiguity case. It also computes the gradients
 function [CINQ, CEQ,  gradCINQ, gradCEQ ] = DynamicConstraintsRU(x,v0,z_,Para)
...
for zTrue=1:2 % true state
    for zReported=1:2 %reprted state
        ...
        cstar = fzero(@(cstar) TerminalPKRU(cstar,bar_vstar(zReported),zTrue,Para),[.001 y-Delta-.001],options);
