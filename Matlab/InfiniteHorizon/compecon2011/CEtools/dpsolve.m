...
% SET PARAMETER & SHOCK DISTRIBUTION DEFAULTS
  ...
  func=model.func;
  params=model.params;
  if nargin<4, v = zeros(size(s,1),1); end
...
% COMPUTE COLLOCATION NODES AND INTERPOLATION MATRIX
  Phi = funbas(fspace,s);           % collocation matrix
  c   = Phi\v;                      % initial basis coefficients
  [F,G,X] = GetFG(s,fspace,model);  % basis functions for discrete states
  
 
 %if ~isfield(model,'actions')
 %  c=valpolicy(fspace,s,x,e,delw,func,params);
 % end
 %disp(norm(v-vmax(s,x,c,fspace,model,F,G,X)))
...
% VMAX - Solves Bellman Equation
  ...
  if isempty(F)
    ...
    usegolden     = optget('dpsolve_vmax','usegolden',0);
    ...
    if usegolden && size(x,2)==1
      ff = @(x) valfunc(c,fspace,s,x,e,delw,func,params);
      [x,v]=golden(ff,xl,xu);
    else
  ...
  % valpolicy determines the value function for a given policy
    function c=valpolicy(fspace,s,x,e,delw,func,params)
    ns = size(s,1);
...
% GETFG - Computes Reward and Discounted Expected Basis for Discrete Choice Models
  ...
  for k=2:K
    kk = k + zeros(ns,1);
    g = feval(func,'g',ss,xx,e(kk,:),params{:});
