...
%   vec           = 1 (0) if f does (not) accept point matrices [0]
...
getopts(options, ...
 ...
 'numits',      200,...     % number ot iterations to perform
...
for i=iters+1:iters+numits                     % loop over each generation 
 ...
 f_old=f_best;
 if showiter
    fprintf('Gen: %4i   gen_last_improvement: %4i   f_best: %15.10f\n',...
         i,gen_last_improvement,f_best); 
 end
 %-------------------------------------------------------------------------
