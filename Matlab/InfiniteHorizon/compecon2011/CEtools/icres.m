...
function [e,c]=icres(y,x,func,params,xindex,fspace,z, ...
  ...
  if xindex(1,1)>=0
    if F(1)==0  
      ...
      H = [H;phi0(1,:)-funbas(fspace,(x(1,2)-x(1,1))/r)];
      f = [f;R(1)-F(1)];
      if xindex(1,1)>0, G = [G;phi1(1,:)]; g = [g;r*R(2)]; end
      if xindex(1,2)>0, G = [G;funbas(fspace,(x(1,2)-x(1,1))/r,1)]; g = [g;-r*R(3)]; end
    end
  end
  if xindex(2,1)>=0
     if F(2)==0
      ...
      H = [H;phi0(2,:)-funbas(fspace,(x(2,2)-x(1,1))/r)];
      f = [f;R(1)-F(2)];
      if xindex(2,1)>0, G = [G;phi1(2,:)]; g = [g;r*R(2)]; end
      if xindex(2,2)>0, G = [G;funbas(fspace,(x(2,2)-x(1,1))/r,1)]; g = [g;-r*R(3)]; end
    end
  end
